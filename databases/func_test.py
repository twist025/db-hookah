...
def print_storage():
    ...
    for st in database['storage']:
        if st['id'] != 0:
            filial = find_by_id_('filials', st['location'])
            storages.append(Table(filial['name'], header=['Название', 'Текущее количество', 'Необходимое количество']))
